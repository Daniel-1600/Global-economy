  ...
  return <WorkspaceShell title="Settings" eyebrow="Workspace">
    <div className="grid gap-6 lg:grid-cols-[220px_1fr]">
      ...
      <motion.section key={active} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="overflow-hidden rounded-3xl border border-white/[0.07] bg-[#0d1320]">
        ...
        {active === "Data" && <div className="p-6 sm:p-8"><div className="rounded-2xl border border-emerald-300/10 bg-emerald-300/[0.04] p-5"><div className="flex items-center gap-2 text-sm font-medium text-emerald-300"><span className="h-2 w-2 rounded-full bg-emerald-300"/>PostgreSQL connected</div><p className="mt-2 text-xs leading-5 text-slate-500">Dashboard and country explorer views read stored GDP records from your configured database.</p></div><div className="mt-5 rounded-2xl border border-white/[0.07] p-5"><p className="text-sm font-medium text-white">Refresh economic data</p><p className="mt-2 text-xs leading-5 text-slate-500">Use the backend import endpoint when you want to pull a fresh World Bank snapshot into PostgreSQL.</p><code className="mt-4 block overflow-x-auto rounded-xl bg-[#080d17] p-4 text-xs text-cyan-300">POST /api/economy/store</code></div></div>}
      </motion.section>
    </div>
  </WorkspaceShell>;
}
